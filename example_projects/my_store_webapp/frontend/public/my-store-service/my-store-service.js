const wasm = '/my-store-service/server.wasm';
let base = '/';
...
export async function run() {
	...
	WebAssembly.instantiateStreaming(source, go.importObject)
	  .then(({ instance }) => {
		go.run(instance);
		console.log('WASM Go server initialized');
  
		self.isGoServerReady = true;
	  })
	  .catch((error) => console.error('Failed to initialize WASM Go server:', error));
